...
counties_list = []
counties_votes ={} #*****goback and check
winning_counties = 0
...
with open(file_to_save, "w") as txt_file:
    ...
    for counties_name in counties_votes:
        ...
        county_vote_count= counties_votes[counties_name]
        # 6c: Calculate the percentage of votes for the county.
        counties_percentage= float(counties_votes[counties_name]) / float(total_votes)*100
        counties_results=(
            f"{counties_name}:{counties_percentage:.1f}% ({county_vote_count:,})\n")
         # 6d: Print the county results to the terminal.
        print(counties_results)
         # 6e: Save the county votes to a text file.
        txt_file.write(counties_results)
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (counties_votes[counties_name] > Largest_votes):
